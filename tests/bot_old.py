...
class ExpenseBot:
    ...
    def _load_last_message_id(self) -> int:
        """Load last processed message ID from a file."""
        try:
    ...
    def _save_last_message_id(self, message_id: int):
        """Save last processed message ID to a file."""
        with open('last_message.txt', 'w') as f:
            f.write(str(message_id))

    async def process_historical_messages(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            print(f"Last processed ID: {self.last_processed_message_id}")
            
            # Only look at last 30 messages if first time, otherwise since last processed
            if self.last_processed_message_id == 0:
                start_id = max(1, current_message_id - 30)
            else:
                start_id = self.last_processed_message_id + 1
                
            print(f"Will check messages from ID {start_id} to {current_message_id}")
            await update.message.reply_text("🔍 Processing historical messages...")

            processed_count = 0
            skipped_count = 0

            try:
                # Get message updates directly
                result = await context.bot.get_updates(
                    offset=start_id - 1,  # Offset should be one less than the message we want
                    limit=100,
                    timeout=30,
                ...
                for update_obj in result:
                    if not update_obj.message or update_obj.message.chat.id != chat_id:
                        continue
                        
                    message = update_obj.message
                    print(f"Processing message: {message.text}")
                    
                    if message.text and any(message.text.startswith(str(i)) for i in range(10)):
                        try:
                            ...
                            if category:
                                ...
                                processed_count += 1
                                print(f"Successfully added expense: {amount} {description}")
                            else:
                                skipped_count += 1
                                print(f"Skipped (no category): {message.text}")
                        except (ValueError, IndexError) as e:
                            skipped_count += 1
                            print(f"Error processing message: {str(e)}")
                            continue

                    self._save_last_message_id(message.message_id)
                    self.last_processed_message_id = message.message_id
            ...
            if processed_count > 0:
                status_message += f"• Added {processed_count} new expense entries\n"
            else:
                status_message += "• No new entries to add\n"
            if skipped_count > 0:
                status_message += f"• Skipped {skipped_count} messages\n"
            
            await update.message.reply_text(status_message)

        except Exception as e:
            print(f"Error processing historical messages: {e}")
            await update.message.reply_text("❌ Error processing historical messages.")
    ...
    try:
      ...
      # In your main() function, update this part:
        app = (
            ...
            .pool_timeout(60.0)          # Increased timeout
            .connect_timeout(60.0)       # Added connect timeout
            .read_timeout(60.0)          # Added read timeout
            .write_timeout(60.0)         # Added write timeout
            .build()
        )
        ...
        app.add_handler(CommandHandler("add", bot.process_historical_messages))
